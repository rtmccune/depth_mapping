...
class DepthPlotter:
    ...
    def process_single_flood_event(self, flood_event, plotting_dir):
        ...
        This method loads virtual sensor depth data, original image rectified data, and observed water level data
        for a specific flood event. It then generates and saves plots showing the depth map and water levels over time.

        Parameters:
        -----------
        flood_event : str
            The name of the flood event folder to be processed.
        plotting_dir : str
            The subdirectory within the flood event's folder where the generated plots will be saved.
        ...
        Notes:
        ------
        - The method loads virtual sensor depth data and observed water levels for the given flood event from Zarr and CSV files.
        - It creates a directory for saving the plots if it does not exist.
        - The `plot_depth_map_and_wtr_levels` method is called to generate the plots, which include depth maps and water levels.
        - This method assumes that the flood event folder contains the necessary Zarr files for depth maps and original image rectifications, as well as the CSV file linking observed water levels to image timestamps.

        Assumptions:
        -------------
        - The Zarr store containing depth maps and the rectified images is properly structured.
        - The CSV file containing water level data and image timestamps is formatted correctly.
        - The `plot_depth_map_and_wtr_levels` method is available for generating the plots.

        This method is called for each individual flood event when processing multiple events (e.g., in parallel or sequentially).
        """
        flood_event_path = os.path.join(self.main_dir, flood_event)

        datetimes, max_depths, avg_depths, vs_depths = self.load_virtual_sensor_depths(
            flood_event_path
        )
