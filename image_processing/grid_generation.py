...
class GridGenerator:
    ...
    """    
    def __init__(self, file_path, min_x_extent=None, max_x_extent=None, min_y_extent=None, max_y_extent=None):
        ...
        self.min_x_extent = min_x_extent if min_x_extent is not None else np.min(self.lidar.x) * 0.3048
        self.max_x_extent = max_x_extent if max_x_extent is not None else np.max(self.lidar.x) * 0.3048
        self.min_y_extent = min_y_extent if min_y_extent is not None else np.min(self.lidar.y) * 0.3048
        self.max_y_extent = max_y_extent if max_y_extent is not None else np.max(self.lidar.y) * 0.3048
    ...
    def create_point_array(self, point_mask_value=2):
        ...
        point_mask = self.lidar.classification == point_mask_value  # Mask points based on classification
        ...
        extent_mask = (
            (xyz_m[0] >= self.min_x_extent) & (xyz_m[0] <= self.max_x_extent) &
            (xyz_m[1] >= self.min_y_extent) & (xyz_m[1] <= self.max_y_extent)
        )
    ...
    def gen_grid(self, resolution, z=0, dir='generated_grids'):
        ...
        grid_x, grid_y = np.mgrid[self.min_x_extent:self.max_x_extent:resolution, self.min_y_extent:self.max_y_extent:resolution]
